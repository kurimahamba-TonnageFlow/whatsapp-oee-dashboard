...
(start_production_run, collect_hourly_update, report_machine_down,
...
it", which this journey explicitly requires. Instead, this script calls
the same real functions run_session() calls, in the same order and with
the same data shape, so the restart can be simulated by simply dropping
the Python `run_state` dict and recovering a fresh one.

Never runs automatically - execute directly:
    python scripts/manual_integration/phase1_mvp_system.py
...
effects (including builtins.input, only ever patched inside run());
all execution lives inside run(), called only from the __main__ guard
below.
"""
...
def apply_hourly_cycle(run_state, scripted_input, label):
    """Mirrors run_session()'s hourly-update branch: collect_hourly_update()
    -> record events -> calculate performance -> check unexplained loss ->
    update in-memory progress -> save Hourly Update history -> persist the
    Production Run snapshot."""
    hourly_update = pulse_main.collect_hourly_update(run_state)

    pulse_main.record_event(
        run_state,
        "Hourly Update",
        "Hourly Update Received",
        run_state["line_technician"],
    ...
    hour_performance = pulse_main.calculate_hour_performance(run_state, hourly_update)

    pulse_main.check_unexplained_production_loss(run_state, hourly_update, hour_performance)

    pulse_main.update_run_progress(run_state, hourly_update)
    ...
    progress_saved = pulse_main.persist_run_progress(run_state)
    checkpoint(f"{label}: progress snapshot saved", True, progress_saved)

    pulse_main.display_hourly_report(run_state, hourly_update, hour_performance)
...
def run():
    require_live_confirmation(
        "phase1_mvp_system.py",
        "Runs a full production journey on the REAL, configured 'GIC' "
        "production line (TEST-labelled customer/product/shift, but a "
        "real factory line) - opens and resolves a fault, completes the "
        "run, and starts a second run on GIC. This is the most "
        "consequential script in this folder.",
    )
    ...
    run_state = pulse_main.start_production_run()
    assert run_state is not None, "Production Run failed to start / save to Supabase"
    ...
    apply_hourly_cycle(run_state, scripted_input, "Hour 1")
    checkpoint("Hour 1: pallets_remaining", 20, run_state["pallets_remaining"])
    checkpoint("Hour 1: total_pallets_completed", 5, run_state["total_pallets_completed"])

    scripted_input.extend("80.0", "5", "None", "no")
    apply_hourly_cycle(run_state, scripted_input, "Hour 2")
    checkpoint("Hour 2: pallets_remaining", 15, run_state["pallets_remaining"])
    checkpoint("Hour 2: total_pallets_completed", 10, run_state["total_pallets_completed"])
    ...
    pulse_main.report_machine_down(run_state)

    checkpoint("Fault opened", 1, len(run_state["open_faults"]))
    fault = run_state["open_faults"][0]
    ...
    apply_hourly_cycle(run_state, scripted_input, "Fault round 1 (engineer called + initial response)")
    ...
    apply_hourly_cycle(run_state, scripted_input, "Fault round 2 (engineering resolves)")
    ...
    checkpoint("Fault still open before technician confirmation", 1, len(run_state["open_faults"]))
    ...
    apply_hourly_cycle(run_state, scripted_input, "Fault round 3 (technician resolves)")

    checkpoint("Fault removed from in-memory open_faults", 0, len(run_state["open_faults"]))
    ...
    scripted_input.extend("80.0", "5", "None", "no")
    apply_hourly_cycle(run_state, scripted_input, "Hour 3")
    checkpoint("Hour 3: pallets_remaining", 10, run_state["pallets_remaining"])
    checkpoint("Hour 3: total_pallets_completed", 15, run_state["total_pallets_completed"])
    ...
    del run_state   # simulate the process being gone - nothing left to reuse

    recovered = pulse_main.recover_production_run(PRODUCTION_LINE)
    ...
    scripted_input.extend("80.0", "5", "None", "no")
    apply_hourly_cycle(recovered, scripted_input, "Hour 4 (post-recovery)")
    ...
    pulse_main.handle_run_completion(recovered)
    ...
    final_progress_saved = pulse_main.persist_run_progress(recovered)
    checkpoint("Final progress persistence succeeded", True, final_progress_saved)

    closed_id = pulse_main.close_production_run(recovered["database_run_id"], pulse_main.current_timestamp())
    ...
    checkpoint("recover_production_run returns None", None, pulse_main.recover_production_run(PRODUCTION_LINE))
    ...
    second_run = pulse_main.start_production_run()
