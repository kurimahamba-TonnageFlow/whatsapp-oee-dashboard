...
line keeps this script re-runnable without ever touching Rovema/GIC/Guill
data or a previous run of this same script.

Never runs automatically - execute directly:
    python scripts/manual_integration/production_run_progress.py
...
def apply_hour(run, pallets_completed, oee=80.0):
    ...
    saved = pulse_main.persist_run_progress(run)
    checkpoint("Progress snapshot write succeeded", True, saved)


def run():
    require_live_confirmation(
        "production_run_progress.py",
        "Creates and mutates a TEST production run on the synthetic "
        "'TEST-LINE-PROGRESS' line, exercising the full progress/restart "
    ...
    checkpoint("Recovered run is the TEST run just created", production_run_id, run_state["database_run_id"])
    checkpoint("Initial pallets_remaining", 38, run_state["pallets_remaining"])
    checkpoint("Initial total_pallets_completed", 0, run_state["total_pallets_completed"])
    checkpoint("Initial potential_overrun_pallets", 0, run_state["potential_overrun_pallets"])
    checkpoint("Initial confirmed_overrun_pallets", 0, run_state["confirmed_overrun_pallets"])

    # ==========================================================
    # STAGE 2: HOUR 1 (+4) -> 34 remaining, 4 completed
    # ==========================================================

    apply_hour(run_state, 4)
    checkpoint("Hour 1 (memory): pallets_remaining", 34, run_state["pallets_remaining"])
    checkpoint("Hour 1 (memory): total_pallets_completed", 4, run_state["total_pallets_completed"])
    ...
    apply_hour(run_state, 5)
    checkpoint("Hour 2 (memory): pallets_remaining", 29, run_state["pallets_remaining"])
    checkpoint("Hour 2 (memory): total_pallets_completed", 9, run_state["total_pallets_completed"])
    ...
    # no reuse of the in-memory `run_state` dict above)
    # ==========================================================
    ...
    recovered_again = pulse_main.recover_production_run(PRODUCTION_LINE)
    ...
    final_recovery = pulse_main.recover_production_run(PRODUCTION_LINE)
    ...
    overrun_recovery = pulse_main.recover_production_run(PRODUCTION_LINE)
    ...
    pulse_main.handle_run_completion(overrun_recovery)
    ...
    post_changeover_recovery = pulse_main.recover_production_run(PRODUCTION_LINE)
